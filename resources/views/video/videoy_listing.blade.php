...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">Watch Video</li>
  </ol>
</nav>
...
<h5 class="flama text-uppercase mb-0"><kbd><i class="fas fa-play"></i> Watch</kbd> Video</h5>            
                 </div>  
                ...
                <iframe width="100%" height="400" src="https://www.youtube.com/embed/{{$ArrViewVideo[0]->vid}}?autoplay=1" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
                <h4 class="flama-font mt-2">{{$ArrViewVideo[0]->title}}</h4>
                <p></p>
                </div>
 ...
 @if(Carbon\Carbon::parse(date_format(date_create($ArrViewVideo[0]->publish_date), 'Y-m-d H:i:s'))->diffInWeeks() > 1)
                            {{(new DateTime(date_format(date_create($ArrViewVideo[0]->publish_date), 'Y-m-d H:i:s')))->format('d-F-Y')}} 
                            @else 
                            {{ Carbon\Carbon::parse(date_format(date_create($ArrViewVideo[0]->publish_date), 'Y-m-d H:i:s'))->diffForHumans()}}
                            @endif</p>
 </div>
 
 <div class="col-md-6 col-lg-6 col-sm-6 col-12 float-left no-padding">
 <ul class="list-inline sm no-padding">
          <li class="list-inline-item"><button type="button" class="btn btn-facebook social-share1 facebook"><i class="fab fa-facebook-f"></i> Share</button></li>
...
<li class="list-inline-item"><button type="button" class="btn btn-secondary social-share facebook"><i class="fas fa-envelope"></i> Mail</button></li>
        </ul>
 </div>
                </div>
                </div>
...
<div class="row mt-5">      
  ...
  <h5 class="panel-title flama text-uppercase tabm"><span class="txt-title">LATEST <span class="pnk">VIDEO</span></span></h5>
</div>
...
target="_blank"><i class="fab fa-whatsapp"></i></a></li><li> <a href="mailto:?subject={{$Photos->title}}{{url('')}}/videos/{{str_slug($Photos->title)}}-{{$Photos->yid}}.html&utm_source=desktop&utm_medium=email&utm_campaign=email&utm_term=email&utm_content=email"
