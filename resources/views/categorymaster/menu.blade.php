...
<div class="main-content">
    <div class="breadcrumb-container">
        ...
        <h2><small><a href="{{url('/')}}/menu?channel={{$currentChannelId}}">Menu Master</a></small></h2>
        <h2> <small> 

         @foreach($parents as $parent)
          <a href="{{url('/')}}/menu?id={{$parent->category_id}}">{{$parent->name}}</a>
            @endforeach
    ...
    <div style="margin-bottom:20px;margin-right:20px;text-align:right;">
        ...
        <a href="{{url('/')}}/menu/create?channel={{$currentChannelId}}">
                        ...
                        $("#selectBoxFilter20").select2();
                           $("#selectBoxFilter20").change(function () {
                            $(this).find("option:selected").each(function () {
                                ...
                                window.location = '{{url("menu")}}' + '?channel=' + $(this).attr("value").trim();
                            }
                    ...
                    <table class="table table-striped" id="tableSortable">
                        ...
                        <tbody>
                            ...
                            <tr id="item_<?php echo $a->id;?>">
                                ...
                                <td>{{$a->id}}</td>
                                <td><a href="{{url('/')}}/menu/{{$a->id}}">{{$a->title}}</a></td>
                                 <td>{{$a->link}}</td>
                                <td>{{$a->created_at}}</td>
                               @if($a->active=='0')
