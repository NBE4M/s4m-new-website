...
@section('title', 'Locked Articles - E4MCMS')
    ...
    <div class="panel">
        <div class="panel-content filler">
            ...
            <div class="panel-header">
                <h1><small>Locked Articles</small></h1>
            ...
            <div class="panel-search container-fluid">
                ...
                <input id="start_date"  required  placeholder="Search" value="{{$_GET['keyword'] or ''}}" type="text" name="keyword">
                <input id="end_date"  required  placeholder="Search" value="{{$_GET['keyword'] or ''}}" type="text" name="keyword">
                </div>-->


                    <button class="btn btn-search" type="submit"></button>
                    @if(isset($_GET['searchin']))
                        <a href="{{url("article/list/published")}}?channel={{$currentChannelId}}@if($selectedNewType)&newstype={{$selectedNewType}} @endif"><button class="btn btn-xs" type="button"><i class="icon-refresh"></i></button></a>
                    @endif
    ...
    <div class="main-content">
        <div class="breadcrumb-container">
            <ul class="xbreadcrumbs">
                ...
                <li class="current">
                    <a href="javascript:;">Locked Articles</a>
                </li>

            </ul>
        </div>           <header>
            <i class="icon-big-notepad"></i>
            <h2><small>Locked Articles</small></h2>
        </header>
    <!--
...
@foreach($newstype as $newstypE)
        <option  @if($newstypE->news_type_id==$selectedNewType) selected="selected" @endif value="{{ $newstypE->news_type_id }}"> {{ $newstypE->name }} </option>
                        @endforeach
            </select>
        </div>
    </div>
...
-->
                        ...
                        <table class="table table-striped" id="tableSortable">
                            ...
                            @foreach($articles as $article)
                                <tr class="gradeX"  id="rowCur{{$article->article_id}}">
                                    <td><a>{{ $article->article_id }}</a> <i class="fa fa-lock"></i></td>
                                    <td>@if($article->locked_by == Session::get('users')->id || Session::get('users')->user_type_id == 1 || Session::get('users')->user_type_id == 2)<a href="{{url('')}}/article/{{ $article->article_id }}"  >{{$article->title}}</a>@elseif($article->locked_by !== '')<a href="#" class="dabled" >{{$article->title}}</a>@endif
                                    </td>
                                    <td><a>{{ $article->username }}</a></td>
                                    <td class="center"><a>{{ $article->publish_date }}</a>
                                        <a>{{ $article->publish_time }}</a>
                                    </td>
    ...
    <script>
        $('.dabled').click(function () {
            ...
            //alert(abc)
            alert('Sorry your are not authorised.')
        })
    </script>
@stop