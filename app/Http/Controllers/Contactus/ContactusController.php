...
class ContactusController extends BaseController
{
    ...
    */
    ...
    {  
        ...
        $contact->name = $request->c_name;
        $contact->email = $request->c_email;
        $contact->subject = $request->c_subject;
        $contact->message = $request->c_message;
        $contact->save(); 
       Session::flash('message', 'Submit your  message successfully'); 
Session::flash('alert-class', 'alert-success'); 
        return back();  // <<<<<<<<< see this line
    }

     public function tags(Request $request)
    {   $currentURL = $request->url();
            if ($currentURL != "https://www.exchange4media.com/tags.html") {
            $href = "https://www.exchange4media.com/tags.html";
    ...
    if ($user->exists) {
        ...
        {
            $message->from('info@exchange4media.com','exchange4media ');
...
public function tags_list(Request $request,$name){
     $tagslist = DB::table('tags')->where('tag', 'LIKE', $name.'%')->orderby('tags_id','desc')->limit(100)->get();
        ...
        foreach ($tagslist as $product) {
             $html.='<a href="https://www.exchange4media.com/tags/'.str_slug($product->tag).'.html" class="btn tags">'.$product->tag.'</a>';
        }
        if ($request->ajax()) {
            return $html;
        }
