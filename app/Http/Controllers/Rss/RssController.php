...
class RssController extends Controller {
    ...
    public function __construct(){
       $this->key=md5(date("dmY").'exchangeformedia'); 
       if(!isset($_GET['key'])){
    ...
    public function individualRss($id) {
        $this->requested_key=$_GET['key']; //md5(date("dmY").'exchangeformedia'); exit;
        ...
        $xml_rss_channel->appendChild( $xml->createElement('title','www.exchange4media.com (Fullstory)')); 
        // Adding description under channel
        $xml_rss_channel->appendChild($xml->createElement('description','News Marketing India advertising Indian brands tv media newspapers')); 
        //Adding language under channel
        $xml_rss_channel->appendChild($xml->createElement('language','en-us'));
        // Adding copyright under channel
        $xml_rss_channel->appendChild($xml->createElement('copyright','Copyright @ 2008 Exchange4media Media Private Limited. All rights reserved'));
        ...
        /* $xml_rss_channel_item->appendChild($xml->createElement('link',$id));*/
         $author=($article['author_type']==1)?'E4M Online Bureau':$article['authorname'];
    ...
    public function latestFullStoryRss() {
        ...
        $xml_urlset_channel = $xml->createElement("channel");
         $xml_urlset_channel->appendChild( $xml->createElement('title','www.exchange4media.com (Fullstory)')); 
         $xml_urlset_channel->appendChild( $xml->createElement('link','https://www.exchange4media.com')); 
        // Adding description under channel
        ...
        //Adding language under channel
        $xml_urlset_channel->appendChild($xml->createElement('language','en-us'));
    ...
    /*for fb*/
     public function facebookinstantarticles() {
      $requested_key = md5(date("dmY").'exchangeformedia');
        //md5(date("dmY").'exchangeformedia'); exit;
     ...
     $articles=  DB::select("select article_id,photopath,title,description,publish_date,category_id,category_name,concat(publish_date,' ',publish_time) as published from `article` group by article_id order by `publish_date` desc limit 100");
         foreach($articles as $article){
          ...
          $urlnew = url(Helper::rscUrl($article->category_name).'/'.Helper::rscUrl($article->title).'_'.$article->article_id.'.html');
                ...
                $xml_urlset_url_head = $xml->createElement('head',' <link rel="canonical" href="https://www.exchange4media.com">
                    <meta charset="utf-8">
                ...
                $xml_urlset_url_address = $xml->createElement('address','<a rel="facebook" href="https://exchange4media.com/author/exchange4media-news-service-49.html">(exchange4media News Service</a>
         content@exchange4media.com');
    ...
    /*for fb*/
     ...
     public function facebookinstantarticlesnew() {
       $requested_key = md5(date("dmY").'exchangeformedia');
        ...
        $xml_urlset->appendChild($attribute_urlset_xmlns);
          $xml_urlset_channel=$xml->createElement('channel');
            $xml_urlset_channel->appendChild($xml->createElement('title','Exchange4media'));
            $xml_urlset_channel->appendChild($xml->createElement('link','https://www.exchange4media.com'));
            $xml_urlset_channel->appendChild($xml->createElement('description','At Exchange4media, read the latest news and updates on advertising agencies, marketing, print, radio, digital, television, media, events and happenings in India.'));
            $xml_urlset_channel->appendChild($xml->createElement('language','en'));
     ...
     $articles=  DB::select("select article_id,photopath,title,description,summary,publish_date,category_id,category_name,concat(publish_date,' ',publish_time) as published from `article` group by article_id order by `publish_date` desc limit 11");
         foreach($articles as $article){
          ...
          $urlnew = url(Helper::rscUrl($article->category_name).'/'.Helper::rscUrl($article->title).'_'.$article->article_id.'.html');
                  ...
                  $xml_urlset_url_item->appendChild($xml->createElement('author','content@exchange4media.com (exchange4media News Service)'));
   ...
   /*mohit rss new*/
       public function latestFullStoryRssnew() {
        ...
        $attribute_uirlset_xmlns_news = $xml->createAttribute('xmlns:news');
        $attribute_uirlset_xmlns_news->value = 'http://www.google.com/schemas/sitemap-news/0.9';
         $xml_urlset_url_link= $xml->createElement('link',htmlspecialchars('exchangeformedia.com',ENT_XML1, 'UTF-8'));
          $xml_urlset_url_category=$xml->createElement('category',htmlspecialchars('India',ENT_XML1, 'UTF-8'));
          $xml_urlset_url_language=$xml->createElement('language',htmlspecialchars('en',ENT_XML1, 'UTF-8'));
          $xml_urlset->appendChild($attribute_urlset_xmlns);
          $xml_urlset->appendChild($attribute_uirlset_xmlns_image);
          $xml_urlset->appendChild($attribute_uirlset_xmlns_news);
       ...
       $articles=  DB::select("select article_id,photopath,title,description,publish_date,category_id,category_name,concat(publish_date,' ',publish_time) as published from `article` group by article_id order by `publish_date` desc limit 10");
         ...
         foreach($articles as $article){
          ...
          $imagelarge=trim($article->photopath)?Config::get('constants.AwsBaseurl').trim($article->photopath):'';
           $imagelargeextra='http://cms.exchange4media.com/files/article/article_extra_large_image/'.trim($article->photopath);
   ...
   /*mohit rss new*/
       ...
       $requested_key = md5(date("dmY").'exchangeformedia');
        ...
        $articles=  DB::select("select article_id,photopath,title,description,publish_date,category_id,url,category_name,concat(publish_date,' ',publish_time) as published from `article` where category_id=$id group by article_id order by published desc limit 25");
         foreach($articles as $article){
            ...
            $xml_urlset_url_news_publication->appendChild($xml->createElement('news:name','Exchange4Media'));
         ...
         /*   
            ...
            $xml_urlset_url_image->appendChild($xml->createElement('image:loc','https://storage.googleapis.com/news-photo/'.htmlspecialchars($image)));
    ...
    public function perYearRss($year) {
        ...
        public function permonthRss($month) {
         $requested_key = md5(date("dmY").'exchangeformedia');
       //echo  md5(date("dmY").'exchangeformedia'); exit;
        if($this->key!=$requested_key){
        echo "Not authorized to access."; exit;            
        }
 ...
 /*alldate sitemap*/
     public function alldatesite() {
            ...
            foreach ($files as $value) {
                ...
                $xml_urlset_url->appendChild($xml->createElement('loc','https://storage.googleapis.com/news-photo/rss/'.pathinfo($value,PATHINFO_BASENAME)));
     ...
     public function happening() {
       ...
       $articles= DB::select("select ar.*,ar_cat.photo_id,ar_cat.photopath from album ar inner join photos ar_cat on ar.id=ar_cat.owner_id  where ar_cat.owned_by = 'album' and ar.valid = '1' group by ar.id order by ar.updated_at desc LIMIT 10");
         foreach($articles as $article){
                    ...
                    $xml_urlset_url_image=$xml->createElement('img' , 'https://storage.googleapis.com/news-photo/album/'.$article->photopath);
                     $xml_urlset_url_src=$xml->createElement('src' , 'https://www.exchange4media.com/photogallery/'.str_slug(htmlspecialchars($article->title), '-').'-'.$article->id.'.html');
...
\Illuminate\Contracts\Filesystem\Filesystem::VISIBILITY_PUBLIC
    ...
    /*google sitemap*/
     public function google_news_sitemap() {
        ...
        $articles=  DB::select("select article_id,photopath,title,description,publish_date,url,category_id,category_name,concat(publish_date,' ',publish_time) as published,tags from `article` order by published desc limit 25");
    ...
    /*google site map*/
     ...
     public function category_news_sitemap() {
        ...
        $articles=  DB::select("select name from `category` where valid = 1 and category_id in (1,3,4,5,6,7,26,80,81,84,111,35,13)  order by category_id desc");
         foreach($articles as $article){
            $xml_urlset_url=$xml->createElement('url');
            $xml_urlset_url->appendChild($xml->createElement('loc',url(Helper::rscUrl($article->name).'-news.html')));
            $xml_urlset->appendChild($xml_urlset_url);
