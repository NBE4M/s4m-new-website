...
class EventLogController extends Controller {
    ...
    public function index(){
     ...
     public function publishScheduledArticle() {
        ...
        ->select(DB::raw('photos.photopath,articles.article_id,articles.title,articles.url,articles.pr_name,articles.pr_email,summary,HOUR(articles.publish_time) as hour'))
          ...
          if (date('H')==$article->hour) {
           $updatearticle = Article::find($article->article_id);
            ...
            $this->sendmail($article->title,$article->article_id,$article->url,$article->summary,$article->photopath);
                if (!empty($article->pr_email)) {
                $pemail = explode(',', $article->pr_email);
                $pemail = array_unique($pemail);
                $pname = explode(',', $article->pr_name);
                $pname = array_unique($pname);
                foreach (array_combine($pname, $pemail) as $key => $to) {
                $this->sendmailtopr($article->title,$article->article_id,$article->url,$to,$key,$article->summary,$article->photopath);
                }
        }
    ...
    public function priview($id){
           $articles = Article::join('authors','articles.author_id','=','authors.author_id')->join('category','articles.category_id','=','category.category_id')->join('photos','articles.article_id','=','photos.owner_id')->select('category.name as cname','authors.name','articles.article_id','photos.photopath','articles.title','articles.summary','articles.description')->where('articles.status','N')->where('articles.article_id',$id)->get();
           if( count( $articles ) == 0)
                {
    ...
    public function sendmail($title,$id,$url,$summary,$photopath){
      ...
      Mail::send('emails.publish_email', ['name' =>'Admin','url'=>$url,'title'=>$title,'summary'=>$summary,'photopath'=>$photopath], function ($message) use ($title) {
     $message->to('e4medit@exchange4media.com')->cc('tech@exchange4media.com')->cc('e4msales@exchange4media.com')->subject('E4M News Alert - '.$title);
      });
