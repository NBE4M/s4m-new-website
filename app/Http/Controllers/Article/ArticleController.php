...
/*guest coloum*/
    ...
    {
        $ArrlistingArticles =  Article::select('article_id','title','url','publish_date','publish_time','photopath','authorname','author_id','summary')->where(DB::raw('CONCAT(article.tags)'),'REGEXP', str_slug($name,' '))->orderby('publish_date','desc')->orderby('publish_time','desc')->paginate(20);
      ...
      public function searcharticle(Request $request)
{
        ...
        $ArrlistingArticles=Article::select('article_id','title','url','publish_date','publish_time','photopath','authorname','author_id','category_name','summary')->whereRaw(
      "MATCH(title,tags) AGAINST(? IN NATURAL LANGUAGE MODE)",array($query))->where('publish_date','!=',null)->orderby('publish_date','desc')->orderby('publish_time','desc')->paginate(20);
        $ArrAuthorDetails='';
        $metatitel = '';
        $ogtitel =  '';
        $ogimage = '';
        ...
        $menus = $this->menus;
         $parents = $this->parents;
        return view('article.search_listing', compact('ArrlistingArticles',
                                                        'menus',
                                                        'parents',
                                                        'arrmostread',
                                                        'ArrMenuSLatestVideo',
